    ...
    (async ()=>{
        ...
        joinButton.onclick = async ()=>{
                    ...
                    setInterval(async ()=>{
                        try {
                            ...
                            for (const [name, position] of Object.entries(positions)){
                                if (userPositions[name]) userPositions[name] = position;
                                else // 位置情報がない場合はデフォルト値を設定
                                userPositions[name] = {
                                    x: 0,
                                    y: 0,
                                    z: 0
                                ...
                                if (mediaElement) adjustVolume(mediaElement, userPositions[myName.textContent], userPositions[name]);
                            }
                        } catch (error) {
            ...
            room.publications.forEach((publication)=>{
                ...
                if (!userPositions[publisherName]) userPositions[publisherName] = {
                    x: 0,
                    y: 0,
                    z: 0
                };
